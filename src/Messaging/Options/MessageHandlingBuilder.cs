...
public abstract class MessagingClientBuilder<TBuilder> : IMessagingClientBuilder where TBuilder : MessagingClientBuilder<TBuilder>
{
    ...
    /// <summary>
    ///     Removes the handler <paramref name="type" />.
    /// </summary>
    /// <param name="type">The message handler implementation type.</param>
    public TBuilder RemoveHandler(Type type)
    {
        if (!type.IsMessageHandler())
            throw new ArgumentException($"Expected message handler but provided {type}.", nameof(type));

        Services.ConfigureMessagingClientOptions(Name, o => o.RemoveHandler(type));
        return Builder;
    }
    ...
    /// <summary>
    ///     Adds the interceptor <paramref name="type" />.
    /// </summary>
    public TBuilder AddInterceptor(Type type)
    {
        if (!type.IsMessageInterceptor())
            throw new ArgumentException($"Expected interceptor but provided {type}.", nameof(type));

        Services.ConfigureMessagingClientOptions(Name, o => o.AddInterceptor(type));
        return Builder;
    }

    /// <summary>
    ///     Adds the interceptor <paramref name="instance" />.
    /// </summary>
    /// <exception cref="ArgumentException"/>
    public TBuilder AddInterceptor(object instance)
    {
        var interceptorType = instance.GetType();
        if (!interceptorType.IsMessageInterceptor())
            throw new ArgumentException($"Expected message interceptor but provided {interceptorType}.", nameof(instance));

        Services.ConfigureMessagingClientOptions(Name, o => o.AddInterceptor(instance));
        return Builder;
    }
