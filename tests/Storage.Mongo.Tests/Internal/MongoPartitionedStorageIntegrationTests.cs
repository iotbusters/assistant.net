...
public class MongoPartitionedStorageIntegrationTests
{
    ...
    {
        var option = await Storage.TryRemove(new(true), upToIndex: 10);
    ...
    public async Task TryRemove_returnsOne_twoKeysExist()
    {
        ...
        var option1 = await Storage.TryRemove(new(true), upToIndex: 1);
        ...
        var option2 = await Storage.TryRemove(new(true), upToIndex: 2);
